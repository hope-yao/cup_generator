...
data_i = build_generator_new_new(z_input, t_input[:,0:1], WB, sig_w)
data = tf.expand_dims(data_i,1)
for i in range(1,num_nodes,1):
    data_i = build_generator_new_new(z_input, t_input[:,i:i+1], WB, sig_w)
...
for ep_i in np.linspace(0., 1., max_epoch):
    ...
    if cnt/1000 < 0.1:
        ...
        w_input_val = [30, 10, 300., 0., 0., 0.]  # height, bottom, area, range, open, stability
        sig_w_val = 1.
    elif cnt/1000 < 0.8:
        w_input_val = [30, 10, 300., 10., 0., 1.]  # height, bottom, area, range, open, stability
        sig_w_val = 5.
    else:
        w_input_val = [30, 10, 300., 10., 0., 1.]  # height, bottom, area, range, open, stability
        sig_w_val = 30.
    ...
    loss_val, center_gravity_val, area_val = sess.run([loss, center_gravity, area_list], feed_dict)
